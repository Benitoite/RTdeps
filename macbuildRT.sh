...
cd ~/doxygen && mkdir build && cd build && cmake .. -DICONV_LIBRARY:FILEPATH=/opt/local/lib/libiconv.dylib -DPYTHON_EXECUTABLE=/usr/bin/python3 && curl https://raw.githubusercontent.com/Benitoite/RTdeps/master/doxygen.patch | git apply && make -j8 && sudo make install
...
cd ~/tiff/tiff* && mkdir build && cd build && cmake .. -DCMAKE_OSX_SYSROOT:PATH=/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX10.9.sdk -DCMAKE_OSX_DEPLOYMENT_TARGET:STRING=10.9 -DCMAKE_BUILD_TYPE:STRING=Release -DCMAKE_INSTALL_PREFIX:PATH=/opt/local && make -j8 && sudo make install
...
cd ~/libpng-1 && mkdir build && cd build && cmake .. -DCMAKE_OSX_SYSROOT:PATH=/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX10.9.sdk -DCMAKE_OSX_DEPLOYMENT_TARGET:STRING=10.9 -DCMAKE_BUILD_TYPE:STRING=Release -DCMAKE_INSTALL_PREFIX:PATH=/opt/local -DZLIB_LIBRARY_RELEASE:FILEPATH=/opt/local/lib/libz.dylib && make -j8 && sudo make install

cd ~/libexpat/expat && mkdir build && cd build && DYLIB_CURRENT_VERSION=8.0.1 DYLIB_COMPATIBILITY_VERSION=7.0.0 CFLAGS="-mmacosx-version-min=10.9" LDFLAGS="-Wl,-headerpad_max_install_names,-undefined,dynamic_lookup,-compatibility_version,7.0.0,-current_version,8.0.1" cmake .. -DCMAKE_OSX_SYSROOT:PATH=/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX10.9.sdk -DCMAKE_OSX_DEPLOYMENT_TARGET:STRING=10.9 -DCMAKE_BUILD_TYPE:STRING=Release -DCMAKE_INSTALL_PREFIX:PATH=/opt/local && DYLIB_CURRENT_VERSION=8.0.1 CFLAGS="-mmacosx-version-min=10.9" DYLIB_COMPATIBILITY_VERSION=7.0.0 LDFLAGS="-Wl,-headerpad_max_install_names,-undefined,dynamic_lookup,-compatibility_version,7.0.0,-current_version,8.0.1" make -j8 && CFLAGS="-mmacosx-version-min=10.9" LDFLAGS="-Wl,-headerpad_max_install_names,-undefined,dynamic_lookup,-compatibility_version,7.0.0,-current_version,8.0.1" sudo make install
...
cd ~/glib-2 && curl https://raw.githubusercontent.com/Homebrew/formula-patches/6164294a75541c278f3863b111791376caa3ad26/glib/hardcoded-paths.diff | git apply && echo "#define _GNU_SOURCE" > glib/glib-unix.c && meson setup --pkg-config-path /opt/local/lib --cmake-prefix-path /opt/local --prefix=/opt/local --buildtype=release _build . -D iconv=external --cross-file ~/maccross --optimization 3 --default-library=shared -D installed_tests=false -D dtrace=false -D bsymbolic_functions=false -D gio_module_dir=/opt/local/lib/gio/modules -D force_posix_threads=true && ninja -C _build && sudo ninja install -C _build && sudo install_name_tool -change libz.1.dylib /opt/local/lib/libz.1.dylib /opt/local/lib/libgio-2.0.0.dylib && sed -i -e "s+5900+$(otool -l /opt/local/lib/libglib-2.0.dylib | grep "current version" | head -1 | tail -c9 | cut -c1-4)+g" glib.la && sudo cp glib.la /opt/local/lib/libglib-2.0.la

cd ~/lensfun && rm -f -r build && mkdir build && cd build && cmake .. -DCMAKE_OSX_SYSROOT:PATH=/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX10.9.sdk -DCMAKE_OSX_DEPLOYMENT_TARGET:STRING=10.9 -DCMAKE_BUILD_TYPE:STRING=Release -DCMAKE_INSTALL_PREFIX:PATH=/opt/local && make -j8 && sudo make install
