...
cd ~/glib && git checkout 2.63.3 && LD=ld CC=clang CXX=clang++ CFLAGS="-L/opt/local/lib -std=c11 -fstrict-aliasing -arch x86_64 -mmacosx-version-min=10.9 -I/opt/local/include -I /opt/local/include/libintl.h" LDFLAGS="-lresolv -bind_at_load -arch x86_64 -mmacosx-version-min=10.9 -L /opt/local/lib" CXXFLAGS="-std=c++11 -I/opt/local/include -arch x86_64 -mmacosx-version-min=10.9" CPPFLAGS="-I/opt/local/include" ac_cv_prog_AWK=/usr/bin/awk ac_cv_prog_GTKDOC_CHECK=  ac_cv_path_GTKDOC_CHECK_PATH= ac_cv_path_GTKDOC_MKPDF= ac_cv_path_GTDOC_REBASE=  PCRE_CFLAGS="-I/opt/local/include" PCRE_LIBS="-L/opt/local/lib -lpcre" meson setup --pkg-config-path /opt/local/lib --cmake-prefix-path /opt/local/lib --libdir=/opt/local/lib --prefix=/opt/local --buildtype=release _build . -D iconv=external --buildtype release --cross-file ~/maccross --optimization 3 -D installed_tests=false && meson configure _build -D iconv=external --buildtype release --optimization 3  --buildtype=release -D nls="enabled" -D installed_tests=false --default-library=static && CC=clang CXX=clang++ CFLAGS="-I /opt/local/include/libintl.h -L/opt/local/lib -std=c11 -fstrict-aliasing -arch x86_64 -mmacosx-version-min=10.9 -I/opt/local/include" LDFLAGS="-lresolv -bind_at_load -arch x86_64 -mmacosx-version-min=10.9 -L/opt/local/lib" CXXFLAGS="-std=c++11 -I/opt/local/include -arch x86_64 -mmacosx-version-min=10.9" CPPFLAGS="-I/opt/local/include" sudo ninja install -C _build && sudo ninja install -C _build
...
cd ~/glibmm && git checkout glibmm-2-62 && sh autogen.sh --prefix=/opt/local --with-sysroot=/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX10.9.sdk --enable-shared=yes 'CFLAGS=-arch x86_64 -mmacosx-version-min=10.9 -I/opt/local/include' 'LDFLAGS=-arch x86_64 -mmacosx-version-min=10.9 -L/opt/local/lib' CXXFLAGS="-arch x86_64 -mmacosx-version-min=10.9" && make -j8 && sudo make install
...
cd ~/pango && git checkout 1.43.0 && meson wrap promote subprojects/glib/subprojects/proxy-libintl.wrap && mkdir build && cd build && meson --prefix=/opt/local --sysconfdir=/etc .. && ninja && sudo ninja install
...
cd ~/gtk &&  meson setup  --prefix=/opt/local --buildtype=release _build .  --cross-file maccross --optimization 3 && meson configure _build -Dtests=false && sudo ninja install -C _build && sudo ninja install -C _build
