...
cd ~/gobject-introspection && ln -s -f /Library/Frameworks/Python.framework/Versions/3.7/include/python3.7m/*.h /opt/local/include && git checkout 1.62.0 && sudo rm -r _build || CFLAGS="-Wl,-rpath,/opt/local/lib" LDFLAGS="-arch x86_64 -mmacosx-version-min=10.9 -L/opt/local/lib" CXXFLAGS="-Wl,-rpath,/opt/local/lib" meson setup _build . --prefix=/opt/local --buildtype release --default-library both --optimization 3 --cross-file ~/maccross && CFLAGS="-Wl,-rpath,/opt/local/lib" LDFLAGS="-arch x86_64 -mmacosx-version-min=10.9 -L/opt/local/lib" CXXFLAGS="-Wl,-rpath,/opt/local/lib" ninja -C _build && sudo ninja -C _build install
...
cd ~/gtk && git checkout gtk-3-24 && git pull && sudo rm -r _build || meson setup  --prefix=/opt/local --buildtype=release _build .  --cross-file ~/maccross --optimization 3  -D tests=false -D win32_backend=false -D wayland_backend=false -D x11_backend=false -D quartz_backend=true -D print_backends=file -D demos=false -D examples=false -D installed_tests=false -D builtin_immodules=no -D introspection=true --libdir /opt/local/lib && ln -s /opt/local/lib _build/lib && ninja -C _build && sudo ninja install -C _build
