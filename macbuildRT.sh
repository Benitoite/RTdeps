...
cd ~/fontconfig && sed -i -e 's+libintl.h+/opt/local/include/libintl.h+g' src/fcint.h && sh autogen.sh --with-libintl-prefix=/opt/local --prefix=/opt/local --with-sysroot=/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX10.9.sdk --enable-shared=yes && make -j8 && sudo make install
